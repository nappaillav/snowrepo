...
class ReplayBuffer():
    ...
    def sample(self, 
        ...
        if include_intermediate:
            ...
            not_done = toTensor(np.where(ind == goal_pos[:, None], 0, 1)[:, :horizon]) # not done
            reward = toTensor(np.where(ind[:, :-1] == goal_pos[:, None], 0, -1)[:, :horizon])
            concat_ind = np.hstack((ind, goal_pos[:, None]))
            ...
            # random_offset = np.random.geometric(p=1 - geom_p, size=batch_size) 
            # random_goal_pos = self.traj_length * tid.reshape(-1) + np.minimum(local_ind[:, 0] - random_offset, self.traj_length - 1)
            # actor_offset = np.random.geometric(p=1 - geom_p, size=batch_size) * np.where(np.random.rand(batch_size)<goal_p_actor, 0, 1)
            ...
            goal = all_state[:,2]
            if self.storage_device:
                return dict(state=state, action=action, next_state=next_state, goal=toDevice(goal),
                            not_done=not_done, reward=reward, weight=weight)
            else:
                return dict(state=toDevice(state), action=toDevice(action), next_state=toDevice(next_state), goal=toDevice(goal),
                            not_done=not_done, reward=reward, weight=weight)
