...
class Encoder(nn.Module):
    def __init__(self, state_dim: int, action_dim: int, pixel_obs: bool,
        ...
        # self.model = nn.Linear(zsa_dim, num_bins + zs_dim + 1) # Change 1
...
class Policy(nn.Module):
    def __init__(self, action_dim: int, discrete: bool, gumbel_tau: float=10, zs_dim: int=512, hdim: int=512, activ: str='relu'):
        super().__init__()
        self.policy = BaseMLP(2*zs_dim, action_dim, hdim, activ) # change 3
    ...
    def forward(self, zs: torch.Tensor, zg:torch.Tensor):
        pre_activ = self.policy(torch.cat([zs,zg],1))
        action = self.activ(pre_activ)
        return action, pre_activ

    # change 5
    def act(self, zs: torch.Tensor, zg:torch.Tensor):
        action, _ = self.forward(zs, zg)
        return action
...
class Value(nn.Module):
    ...
    # change 7
    def forward(self, zsa: torch.Tensor, zg:torch.Tensor):
        
        return torch.cat([self.q1(torch.cat([zsa, zg], 1)), 
                        self.q2(torch.cat([zsa, zg], 1))], 1)
