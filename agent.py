...
class Agent:
    def __init__(self, obs_shape: tuple, action_dim: int, max_action: float, pixel_obs: bool, discrete: bool,
        ...
        self.replay_buffer = ReplayBuffer(self.batch_size, weight=None, gamma=self.discount, 
        ...
        if self.value_loss_fn == 'BCE':
            assert self.gc_negative == False

        encoder_modules = functools.partial(models.Encoder, state_dim=obs_shape[0], pixel_obs=pixel_obs, 
    ...
    def train_critic(self, batch):
        ...
        with torch.no_grad():
            ...
            if self.s_enc == False:
                ...
                goal_rep = self.encoder.goal_emb(zs, zg)
        
        goal = batch['value_goal'] if self.s_enc else goal_rep # detach the encoder goal
