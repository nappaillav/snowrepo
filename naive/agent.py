...
class Agent:
    def __init__(self, obs_shape: tuple, action_dim: int, max_action: float, pixel_obs: bool, discrete: bool,
        ...
        if self.hp.batched_buffer:
            ...
            self.replay_buffer = buffer.ReplayBufferBatch(self.batch_size, weight=None, gamma=self.discount, 
        ...
        self.encoder = models.Encoder(obs_shape[0], action_dim, pixel_obs,
    ...
    def train(self):
        ...
        # if self.hp.batched_buffer and self.training_steps % 5000 == 0:
        ...
        if (self.training_steps-1) % self.target_update_freq == 0:
            ...
            for _ in range(self.target_update_freq):
                ...
                self.train_encoder(state, action, next_state, reward, not_done, False)
        ...
        state, action, next_state, reward, not_done, goal = batch['state'],batch['action'],batch['next_state'],batch['reward'], batch['not_done'], batch['value_goal'] 
        state, next_state = maybe_augment_state(state, next_state, self.pixel_obs, self.pixel_augs)
        reward, term_discount = multi_step_reward(reward, not_done, self.discount)

        rl_metrics = self.train_rl(state, action, next_state, reward, goal, term_discount,
    ...
    def train_rl(self, state: torch.Tensor, action: torch.Tensor, next_state: torch.Tensor,
        reward: torch.Tensor, goal:torch.Tensor, term_discount: torch.Tensor, reward_scale: float, target_reward_scale: float):
        metrics = {}
        with torch.no_grad():
            next_zs = self.encoder_target.zs(next_state)
            zg = self.encoder_target.zs(goal)
            noise = (torch.randn_like(action) * self.target_policy_noise).clamp(-self.noise_clip, self.noise_clip)
            next_action = realign(self.policy_target.act(next_zs, zg) + noise, self.discrete) # Clips to (-1,1) OR one_hot of argmax.

            next_zsa = self.encoder_target(next_zs, next_action)
            Q_target = self.value_target(next_zsa, zg).min(1,keepdim=True).values
        ...
        policy_action, pre_activ = self.policy(zs, zg)
        zsa = self.encoder(zs, policy_action)
        Q_policy = self.value(zsa,zg)
        policy_loss = -Q_policy.mean()
        pre_loss = self.pre_activ_weight * pre_activ.pow(2).mean()
        # BC Loss
        bc_loss = self.hp.lmbda * F.mse_loss(policy_action , action)
