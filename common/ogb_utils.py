...
class CustomDataloader:
    ...
    def load_dataset_chunked(self, pos):
        ...
        # memory, _ = torch.cuda.mem_get_info()
        # state_shape = obs_chunk.shape[1:]
        # obs_space = np.prod((obs_chunk.shape[0], *state_shape), dtype=np.int64) * (1 if ob_dtype == np.uint8 else 4)
        # act_space = obs_chunk.shape[0] * (act_chunk.shape[-1] + 2) * 4
        # storage_device = torch.device('cuda') if not storage_cpu else torch.device('cpu')
        storage_device = torch.device('cpu')
        
        # Convert to tensor
        # chunk = {
        #     'observations': torch.tensor(obs_chunk, dtype=torch.uint8 if ob_dtype == np.uint8 else torch.float32, device=storage_device),
        #     'actions': torch.tensor(act_chunk, dtype=torch.float32, device=storage_device),
        #     'terminals': torch.tensor(new_terms, dtype=torch.float32, device=storage_device),
        #     'valids': torch.tensor(valids, dtype=torch.float32, device=storage_device),
        # }
