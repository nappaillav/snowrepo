...
class Hyperparameters:
    ...
    value_loss_fn: str = 'smooth_l1' # expectile
...
class Agent:
    def __init__(self, obs_shape: tuple, action_dim: int, max_action: float, pixel_obs: bool, discrete: bool,
        ...
        # High Level Policy
        high_input = self.zs_dim + self.goal_dim if self.goal_dim > 0 else 2 * self.zs_dim
        ...
        # Low Level Policy
        self.low_policy = models.Actor(high_input, action_dim, discrete, self.gumbel_tau, self.low_policy_hdim, self.low_policy_activ,
        ...
        # Critic
        self.critic = models.Value(self.zsa_dim, self.value_hdim, self.value_activ).to(self.device)
        self.critic_optimizer = torch.optim.AdamW(self.critic.parameters(), lr=self.value_lr, weight_decay=self.value_wd)
    ...
    def select_action(self, state: np.array, goal: np.array):
        ...
        with torch.no_grad():
            ...
            if self.high_stochastic:
                sub_goal = high_dist.sample()
            else:
                sub_goal, _ = high_dist
            ...
            action = self.low_policy.act(zs, sub_goal)            
    ...
    def train_rl(self, state: torch.Tensor, action: torch.Tensor, next_state: torch.Tensor,
        ...
        with torch.no_grad():
            ...
            zsa = self.encoder_target(zs, action)
            q_target = self.critic_target(zsa, zg).min(1, keepdim=True).values
            
            # if self.gc_negative:
            #     Q_target.clamp(-1/(1-self.discount), 0)
            # else:
            #     Q_target.clamp(0, 1)

            zs = self.encoder.zs(state)
            zg = self.encoder.zs(value_goal)
        
        v = self.value(zs, zg)#.mean(1, keepdim=True)
        # value_loss = expectile_loss(q_target-v, q_target-v, self.expectile).mean()
        value_loss = F.smooth_l1_loss(v, q_target.expand(-1,2))
        ...
        metrics.update({
            ...
            'train/Value_Vmean': v.mean().item(),
            'train/Value_Vmax': v.max().item(),
            'train/Value_Vmin': v.min().item(),
        ...
        """
        with torch.no_grad():
            next_zs_t = self.encoder_target.zs(next_state)
            zg_t = self.encoder_target.zs(value_goal)
            
            next_v = self.value_target(next_zs_t, zg_t).min(1,keepdim=True).values
            Q_target = reward + term_discount * next_v 

            # if self.gc_negative:
            #     Q_target = Q_target.clamp(-1/(1-self.discount), 0)
            # else:
            #     Q_target = Q_target.clamp(0, 1)

            # zs = self.encoder.zs(state)
            # zg = self.encoder.zs(value_goal)
            zsa = self.encoder(zs, action)

        Q = self.critic(zsa, zg)
        
        if self.value_loss_fn == 'smooth_l1':
            # MRQ LOSS : Smooth L1 is symetric
            critic_loss = F.smooth_l1_loss(Q, Q_target.expand(-1,2))
        else:
            # IQL
            critic_loss = ((Q[:,0] - Q_target.squeeze(1)) ** 2 + (Q[:,1] - Q_target.squeeze(1)) ** 2).mean()

        self.critic_optimizer.zero_grad(set_to_none=True)
        critic_loss.backward()
        norm = torch.nn.utils.clip_grad_norm_(self.critic.parameters(), self.value_grad_clip)
        self.critic_optimizer.step()
        metrics.update({
            # if critic is normal we can remove it 
            'train/critic_loss': critic_loss.item(),
            'train/critic_Qmean': Q.mean().item(),
            'train/critic_Qmax': Q.max().item(),
            'train/critic_Qmin': Q.min().item(),
            'train/critic_Qnorm': norm.item(),            
        })
        ...
        #     "train/Lactor_bcloss": log_prob.mean().item(),    
        # })
        # Low level Update
        with torch.no_grad():
            zg_sub = self.encoder.zs(actor_sub_goal)
        ...
        policy_action, pre_activ = self.low_policy(zs, goal_rep) # pre_activ is missing
        zsa = self.encoder(zs, policy_action)
        Q_policy = self.critic(zsa,zg)
        
        policy_loss = -Q_policy.mean()
        pre_loss = self.pre_activ_weight * pre_activ.pow(2).mean()
        low_bc_loss = self.hp.lmbda * F.mse_loss(policy_action , action)
        low_loss = policy_loss + Q_policy.abs().mean().detach()*low_bc_loss + pre_loss
        
        self.low_policy_optimizer.zero_grad(set_to_none=True)
        low_loss.backward()
        self.low_policy_optimizer.step()
        metrics.update({
                "train/Lactor_loss": low_loss.item(),
                "train/Lactor_policy_loss": policy_loss.item(),
                "train/Lactor_bc_loss": low_bc_loss.item(),
                "train/Lactor_pre_loss": pre_loss.item(),
                'train/Lactor_Qmean': Q_policy.mean().item(),
                'train/Lactor_Qmax': Q_policy.max().item(),
                'train/Lactor_Qmin': Q_policy.min().item(),
            })
        ...
        if self.high_stochastic:
            ...
            # goal_dim == 0 then only its possible
            assert self.goal_dim == 0, "Run only when your High policy predicts the zg"
            with torch.no_grad():
                zg_actor = self.encoder.zs(actor_goal)
                # exp_a = torch.exp(adv * self.high_alpha).clamp(max=100.0)

            # use low level policy with action as this 
            pred_goal_rep, _ = self.high_policy(zs, zg_actor)
            v = self.value(zs, zg_actor).mean(1, keepdim=True)
            nv = self.value(zg_sub, zg_actor).mean(1, keepdim=True)
            adv = nv - v
            adv_loss = -adv.mean()
            # pre_loss = self.pre_activ_weight * pre_activ.pow(2).mean()
            high_bc_loss = self.hp.lmbda * F.mse_loss(pred_goal_rep, goal_rep)

            # log_prob = dist.log_prob(goal_rep)
            high_actor_loss = adv_loss + adv.abs().mean().detach()*high_bc_loss
            ...
            'train/Hactor_loss': high_actor_loss.item(),
            'train/Hactor_adv': adv.mean().item(),
            'train/Hactor_bcloss': high_bc_loss.mean().item(),
            # 'train/Hactor_mse': torch.mean((dist.mean - zg_sub) ** 2).item(),
            # 'train/Hactor_std': torch.mean(dist.scale).item(),
            })
