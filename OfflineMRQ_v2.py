...
class Agent:
    def __init__(self, obs_shape: tuple, action_dim: int, max_action: float, pixel_obs: bool, discrete: bool,
        ...
        # Same encoder
        if self.use_tdmpc:
            print('Using TDMPC World model')
            self.encoder = worldmodel.Encoder(obs_shape[0] * history, action_dim, pixel_obs,
                self.num_bins, self.zs_dim, self.za_dim, self.zsa_dim,
                self.enc_hdim, self.enc_activ).to(self.device)
            self.pixel_aug = False
        else:
    ...
    def train(self):
        ...
        if (self.training_steps-1) % self.target_update_freq == 0:
            ...
            for _ in range(self.target_update_freq):
                ...
                state, action, next_state, reward, not_done = self.replay_buffer.sample(horizon=self.enc_horizon, include_intermediate=True)
                state, next_state = maybe_augment_state(state, next_state, self.pixel_obs, self.pixel_augs)
                enc_loss += self.train_encoder(state, action, next_state, reward, not_done, False)
    ...
    def train_rl(self, state: torch.Tensor, action: torch.Tensor, next_state: torch.Tensor, goal:torch.Tensor,
        ...
        with torch.no_grad():
            ...
            goal_zs = self.encoder_target.zs(goal)

            noise = (torch.randn_like(action) * self.target_policy_noise).clamp(-self.noise_clip, self.noise_clip)
            next_action = realign(self.policy_target.act(next_zs, goal_zs) + noise, self.discrete) # Clips to (-1,1) OR one_hot of argmax.

            next_zsa = self.encoder_target(next_zs, next_action)
            Q_target = self.value_target(next_zsa, goal_zs).min(1,keepdim=True).values
        ...
        policy_action, pre_activ = self.policy(zs, goal_zs)
        zsa = self.encoder(zs, policy_action)
        Q_policy = self.value(zsa, goal_zs)
        ...
        metrics.update({
                ...
                'train/actor_Lactiv': pre_loss.item(),
                'train/actor_BCloss': bc_loss.item(),
                'train/actor_TotalLoss':loss.item(),
